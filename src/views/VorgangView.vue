<template>
  <div class="typed-folder-view vorgang-view">
    <div class="typed-folder-header">
      <span class="typed-folder-type-badge">Vorgang</span>
      <span v-if="fileRef && showAktzInName" class="typed-folder-aktz">{{ fileRef }}</span>
      <button v-if="canCreate" class="typed-action-btn" @click="createChild('register')">
...
.typed-folder-type-badge { font-size: 12px; font-weight: 600; padding: 2px 8px; border-radius: 4px; background: #fff3e0; color: #e65100; }
.typed-folder-aktz { font-size: 13px; font-weight: 600; color: #1a73e8; font-family: monospace; }
.typed-action-btn { display: inline-flex; align-items: center; gap: 4px; padding: 6px 14px; border: 1px solid #e65100; border-radius: 6px; background: #e65100; color: white; font-size: 13px; cursor: pointer; }
